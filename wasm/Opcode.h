...
struct Opcode {
	...
	enum Enum : uint32_t {
#define WABT_OPCODE(rtype, type1, type2, type3, mem_size, prefix, code, Name, text) Name,
#include "Opcode.hpp"
#undef WABT_OPCODE
		Invalid,
	};
...
#define WABT_OPCODE(rtype, type1, type2, type3, mem_size, prefix, code, Name, text) static Opcode Name##_Opcode;
#include "Opcode.hpp"
#undef WABT_OPCODE

	Opcode() = default;  // Provided so Opcode can be member of a union.
	Opcode(Enum e) : _enum(e) { }
	...
	bool IsInvalid() const { return _enum >= Invalid; }
...
private:
	...
	static Enum EncodeInvalidOpcode(uint32_t prefix_code) {
		...
		WASM_ASSERT(result >= Invalid);
		return result;
	}

	static void DecodeInvalidOpcode(Enum e, uint8_t* out_prefix, uint32_t* out_code) {
