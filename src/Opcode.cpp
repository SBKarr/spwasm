...
// static
Opcode::Info Opcode::_infos[] = {
#define WABT_OPCODE(rtype, type1, type2, type3, mem_size, prefix, code, Name, text) \
	{text, Type::rtype, Type::type1, Type::type2, Type::type3, \
...
Opcode Opcode::FromCode(uint8_t prefix, uint32_t code) {
	...
	auto begin = _infos;
	auto end = _infos + WABT_ARRAY_SIZE(_infos);
	...
	return Opcode(static_cast<Enum>(iter - _infos));
}
...
Opcode::Info Opcode::GetInfo() const {
	...
	uint8_t prefix;
	uint32_t code;
	DecodeInvalidOpcode(_enum, &prefix, &code);
	const Info invalid_info = { "<invalid>", Type::Void, Type::Void, Type::Void,
			Type::Void, 0, prefix, code, PrefixCode(prefix, code), };
	return invalid_info;
}
...
bool Opcode::IsEnabled(const Features& features) const {
	...
	case Opcode::CatchAll:
		return features.isExceptionsEnabled();
	...
	case Opcode::I64TruncUSatF64:
		return features.isSatFloatToIntEnabled();
